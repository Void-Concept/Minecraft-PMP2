# progress counter for multiple mechanics
scoreboard objectives remove progress
scoreboard objectives add progress dummy

# wall jumping cooldown timer
scoreboard objectives remove walljump
scoreboard objectives add walljump dummy
scoreboard objectives remove wallJumpFace
scoreboard objectives add wallJumpFace dummy

# wall running cooldown timer
scoreboard objectives remove wallrun
scoreboard objectives add wallrun dummy
scoreboard objectives remove wallruncooldown
scoreboard objectives add wallruncooldown dummy

scoreboard teams remove nocollision
scoreboard teams add nocollision
scoreboard teams option nocollision collisionRule never
